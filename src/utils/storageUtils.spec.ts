...
import { getExtention, uploadAvatar, uploadEventPicture, uploadEventBanner } from './storageUtils';
...
describe('uploadAvatar', () => {
  ...
  it('should reject non-image mime types', async () => {
    ...
    await expect(uploadAvatar(file, userId)).rejects.toThrow('Invalid file type');
  });
  ...
  it('should accept image files', async () => {
    ...
    vi.mocked(fileService.uploadFile).mockResolvedValue({ id:"123",path: 'avatars/user123.jpg',fullPath: 'avatars/user123.jpg' });
  ...
  it('should call uploadFile with correct path', async () => {
    ...
    vi.mocked(fileService.uploadFile).mockResolvedValue({ path: 'avatars/user456.png', fullPath: 'avatars/user456.png', id: '456' });

    await uploadAvatar(file, userId);

    expect(fileService.uploadFile).toHaveBeenCalledWith('avatars/user456.png', file);
  });
  ...
  it('should return public URL for avatar', async () => {
    ...
    vi.mocked(fileService.uploadFile).mockResolvedValue({ path: 'avatars/user789.jpg', fullPath: 'avatars/user789.jpg', id: '789' });
...
describe('uploadEventPicture', () => {
  ...
  it('should reject non-image files', async () => {
    ...
    await expect(uploadEventPicture(file, eventId)).rejects.toThrow(BadRequestError);
  });

  it('should reject files larger than 50MB', async () => {
    // Create a mock file that reports size as 51MB
    const file = new File(['x'.repeat(51e7)], 'large.jpg', { type: 'image/jpeg' });
    const eventId = 'event123' as any;

    await expect(uploadEventPicture(file, eventId)).rejects.toThrow(BadRequestError);
    await expect(uploadEventPicture(file, eventId)).rejects.toThrow('För stor fil');
  });
  ...
  it('should accept valid image files under size limit', async () => {
    ...
    vi.mocked(fileService.uploadFile).mockResolvedValue({ path: 'events/event123.jpg', fullPath: 'events/event123.jpg', id: '123' });
  ...
  it('should call uploadFile with event path', async () => {
    ...
    vi.mocked(fileService.uploadFile).mockResolvedValue({ path: 'events/event456.png', fullPath: 'events/event456.png', id: '456' });

    await uploadEventPicture(file, eventId);

    expect(fileService.uploadFile).toHaveBeenCalledWith('events/event456.png', file);
  });
  ...
  it('should return public URL for event picture', async () => {
    ...
    vi.mocked(fileService.uploadFile).mockResolvedValue({ path: 'events/event789.jpg', fullPath: 'events/event789.jpg', id: '789' });
  ...
  it('should handle upload errors', async () => {
    ...
    await expect(uploadEventPicture(file, eventId)).rejects.toThrow(InternalError);
  });
...
describe('uploadEventBanner', () => {
  ...
  it('should upload file with banner suffix', async () => {
    ...
    vi.mocked(fileService.uploadFile).mockResolvedValue({ path: 'events/event123-banner.jpg', fullPath: 'events/event123-banner.jpg', id: '123' });

    await uploadEventBanner(file, eventId);

    expect(fileService.uploadFile).toHaveBeenCalledWith('events/event123-banner.jpg', file);
  });
  ...
  it('should return public URL for banner', async () => {
    ...
    vi.mocked(fileService.uploadFile).mockResolvedValue({ path: 'events/event456-banner.jpg', fullPath: 'events/event456-banner.jpg', id: '456' });
  ...
  it('should reject non-image files', async () => {
    const file = new File(['content'], 'banner.pdf', { type: 'application/pdf' });
    const eventId = 'event123' as any;
  ...
  it('should reject oversized files', async () => {
    const file = new File(['x'.repeat(51e7)], 'banner.jpg', { type: 'image/jpeg' });
    const eventId = 'event123' as any;
